...
- **counterexample_ledger.md** — 0 反例铁律的**持续验证机制**（每条死规则必带反例账本 + 反思 hook, 出 1 个反例就反思）。**硬性配套**, 立死规则必走。
- **information_sufficiency.md** — 信息充分性的**操作手册**（F-14/Dry Run/Agent Retry 等机制）。本文是其思想基础。
- **format.md** — Format 设计时同样适用原则 1：不要在 Format description 里写"LLM 没验证过的规则"。
- **pipeline.md** — 节点拓扑决策也适用：`LLMRouter` vs `Router` 的选择应按原则 1 默认偏向前者，除非确定性是铁律。
...
- 100% 必做字段 → schema `required` + `minLength` (按 feedback_100pct_required_goes_to_skeleton)
