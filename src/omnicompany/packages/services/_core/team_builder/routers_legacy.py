...
"""team_builder routers_legacy — workflow_factory 全部 10 个节点的 Router 实现 (Stage 3 正式位置).

2026-07-26 OMNI-040 Stage 3 清洁: 本文件从 `_archive/` 迁回 team_builder 顶层正式位置,
`_archive/` 不再承载活业务代码. workers/*.py 的 XxxWorker 经 Diamond 继承本文件的 XxxRouter.

节点分布：
  HARD (确定性): E compile_checker, G error_route_auditor, H integration_tester, J finalizer
  LLMRouter:     A req_analyzer, B format_designer, C node_planner, E' syntax_fixer
  AgentNodeLoop: D code_generator, I auto_fixer

注: 原 F 节点 (LAP 九维检查器) 已于 2026-07-03 批4 显式废止, 实现体留在
`_archive/routers_legacy.py` 作历史参考, 活代码无任何引用.
"""
...
class CompileCheckerRouter(Router):
    ...
    def run(self, input_data: Any) -> Verdict:
        ...
        if report["l1_syntax"]["passed"] and pkg_path:
            ...
            # __file__ = .../src/omnicompany/packages/services/_core/team_builder/routers_legacy.py
            # parents: [0]=team_builder [1]=_core [2]=services [3]=packages [4]=omnicompany [5]=src
...
{
  "goal": "工作流要达成的目标（一句话）",
  "domain": "所属领域（sw/research/rewrite/custom 等）",
...
- 参考现有管线：sw-verify, sw-review, sw-plan, sw-tdd, sw-implement, debug,
  lang-rewrite, equiv-test, guardian, domain-learn, skill-import
"""
...
class FinalizerRouter(Router):
    ...
    def run(self, input_data: Any) -> Verdict:
        ...
        # ── 写入文件系统 ──
        # pkg_path 如 "omnicompany.packages.domains.example_domain.tavern_pool_modify"
        # → 写到 src/omnifactory/packages/domains/example_domain/tavern_pool_modify/
        written = False
