...
class ExternalAgentProviderV1(_StrictModel):
    ...
    def require_placeholders(cls, value: list[str], info: Any) -> list[str]:
        ...
        missing = sorted(required.difference(joined))
        if missing:
...
def _command(
    command_id: str,
    suffix: str,
    category: PlayerCLICommandV1.model_fields["category"].annotation,
    summary: str,
...
def build_player_facility_contract() -> PlayerFacilityContractV1:
    commands = [
        ...
        _command("environment.use", "environment use", "环境", "选择已验证环境叶。", "local_state"),
        ...
        _command("observe.capture", "observe capture", "观察", "采集当前画面、UI 树与必要运行态。", "local_state"),
        ...
        _command("task.claim", "task claim", "任务", "认领任务并绑定 Session。", "local_state"),
        ...
        _command("skill.validate", "skill validate", "技能", "在适用环境内验证技能速度、精度和副作用。", "device"),
        _command("guide.search", "guide search", "攻略", "搜索有时效和来源的外部攻略。"),
        ...
        _command("evidence.prune", "evidence prune", "证据", "按去重策略清理无变化静帧。", "local_state"),
        _command("benchmark.list", "benchmark list", "基准", "列出 AFKJ 与持续 Session 基准。"),
        _command("benchmark.run", "benchmark run", "基准", "运行有预算的同口径基准。", "device"),
    ...
    payload: dict[str, Any] = {
        ...
        "continuous_session_policy": ContinuousSessionPolicyV1(
            ...
            allowed_restart_reasons=[
                "phase_complete", "hard_reset", "environment_identity_change",
        ...
        "invariants": [
            ...
            "已知技能走低级快速自动化，新内容由持续 Agent 步进探索并及时结晶技能。",
