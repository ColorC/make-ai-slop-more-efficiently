/**
 * entities/review/businesses/narrativeToolbars — 叙事渲染器的业务顶栏工厂(件一)。
 *
 * 2026-07 首屏拆包: 从 narrative.tsx 剥离。工厂本身轻量(纯函数 + 自洽 NarrativeRulings),
 * 启动期经 businesses/index.ts 静态挂到各 RendererEntry.toolbar; 渲染器本体
 * (narrative.tsx 九个 View) 保持 React.lazy, 不进首屏静态图。
 *
 * 现协议: 每个叙事渲染器把自己的 icon/title/sub/适用裁决/决策历程导出成一个 toolbar 工厂
 * (narrativeToolbar), 由 businesses/index.ts 挂到 RendererEntry.toolbar; MaterialDetail 解析后
 * 渲染成单条合并顶栏。适用裁决(需异步取域层级 + 自带展开态)封装为自洽活体组件 NarrativeRulings,
 * 经 spec.aux 挂进合并栏中间区。决策历程按钮的跳转行为原样保留(经 usePanels.getState().openTab)。
 */
...
const ST: Record<string, CSSProperties> = {
  // 顶栏其余件(图标/标题/跳转)已随件一并入 MaterialDetail 合并栏; 这里仅保留"适用裁决"chip 与其展开浮层。
  rulChip: {
    display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 999, cursor: 'pointer',
    fontSize: 12, color: 'var(--fp-accent)', background: 'color-mix(in srgb, var(--fp-accent) 10%, transparent)',
    border: '1px solid color-mix(in srgb, var(--fp-accent) 32%, transparent)',
  },
  ...
  rulQuote: {
    ...
    borderLeft: '2px solid color-mix(in srgb, var(--fp-accent) 55%, transparent)',
    padding: '4px 10px', background: 'color-mix(in srgb, var(--fp-accent) 6%, transparent)', borderRadius: '0 6px 6px 0',
  },
}
