#!/usr/bin/env node
// [OMNI fork-patch omni/chat-backend] Windows 前台窗口硬约束: 必须早于任何 SDK/provider import,
// 给 child_process spawn 族注入 windowsHide。见 server/_omni-windows-hide.js。
import './_omni-windows-hide.js';
...
async function startServer() {
    try {
        ...
        // [OMNI fork-patch omni/chat-backend] 单用户·免账号模式: omnidashboard 把 CCUI 当内部
        // 聊天后端(dashboard 控制面 8200 才是门面), 不再隔一道登录。平台模式(VITE_IS_PLATFORM=true)
        // 下 auth 中间件(REST + WebSocket)全 bypass 取 getFirstUser()(见 server/middleware/auth.js),
        // 故库里保证有一个默认用户即可。密码在平台模式永不校验, hash 占位即可。
        if (IS_PLATFORM && !userDb.hasUsers()) {
            userDb.createUser('omni', 'omni-platform-no-login');
            console.log('[omni] platform mode: seeded default single user "omni" (no-login core)');
        }
