...
async function checkTaskMasterInstallation() {
    // [OMNI] taskmaster 休眠: omnidashboard 不接外部 task-master CLI(任务系统将绑自家
    // boss_sight/governance, 见 plan 债务)。这里短路返回"未安装·休眠", 不再 spawn 探测
    // 外部工具。整套 taskmaster 代码保留作参考(面板 UI / PRD→任务图 思路)。
    // 原 `which task-master` + `task-master --version` 探测逻辑见 git 历史。
    return {
        isInstalled: false,
        installPath: null,
        version: null,
        reason: 'omni: taskmaster dormant (not wired to external task-master CLI)',
        dormant: true,
    };
}
