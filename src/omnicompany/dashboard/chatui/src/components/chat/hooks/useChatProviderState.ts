...
export function useChatProviderState({ selectedSession, selectedProject }: UseChatProviderStateArgs) {
  ...
  const [provider, setProvider] = useState<LLMProvider>(() => {
    // [OMNI] 深链支持: 嵌入方(如驾驶舱"总控对话"落点)可用 ?provider=controller
    // 预选 provider, 优先于 localStorage 记忆值。仅在首次挂载读一次。
    try {
      const urlProvider = new URLSearchParams(window.location.search).get('provider');
      if (urlProvider) return urlProvider as LLMProvider;
    } catch { /* SSR / privacy mode 兜底 */ }
  ...
  const [controllerModel, setControllerModel] = useState<string>(() => {
    return localStorage.getItem('controller-model') || FALLBACK_DEFAULT_MODEL.controller;
  });
  ...
  const setStoredProviderModel = useCallback((targetProvider: LLMProvider, model: string) => {
    ...
    localStorage.setItem('controller-model', model);
  }, []);

  const loadProviderModels = useCallback(async (options: { bypassCache?: boolean } = {}) => {
    const providers: LLMProvider[] = ['claude', 'cursor', 'codex', 'gemini', 'kimi', 'opencode', 'omni_agent', 'controller'];
  ...
  useEffect(() => {
    ...
    if (omniAgent) {
      ...
      if (next !== omniAgentModel) {
        setOmniAgentModel(next);
      }
      if (localStorage.getItem('omni_agent-model') !== next) {
        localStorage.setItem('omni_agent-model', next);
      }
    }
  }, [providerModelCatalog.omni_agent, omniAgentModel]);

  useEffect(() => {
    const controller = providerModelCatalog.controller;
    if (controller) {
      const next = pickStoredOrCurrent('controller-model', controllerModel, controller);
      if (next !== controllerModel) {
        setControllerModel(next);
      }
      if (localStorage.getItem('controller-model') !== next) {
        localStorage.setItem('controller-model', next);
      }
    }
  }, [providerModelCatalog.controller, controllerModel]);
