...
---
omnikb_type: karch
id: kb.arch.pipeline.pattern_discovery
name: 'Pipeline: pattern-discovery'
tags:
- layer.pipeline
- topic.pipeline
...
- domain.workflow
maturity: draft
summary: 后台模式发现 — 从行为保全摘要中聚类发现重复模式 → 自动触发轨迹归纳
scope: omnicompany
code_anchors:
- src/omnicompany/core/pipelines.py:L575-L600
---

# Pipeline: pattern-discovery

> **已知概要**: 后台模式发现 — 从行为保全摘要中聚类发现重复模式 → 自动触发轨迹归纳

## Identity

| Field | Value |
|---|---|
| Pipeline name | `pattern-discovery` |
| Domain tag | `workflow` |
