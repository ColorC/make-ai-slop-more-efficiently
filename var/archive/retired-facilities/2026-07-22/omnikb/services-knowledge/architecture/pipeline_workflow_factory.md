...
---
omnikb_type: karch
id: kb.arch.pipeline.workflow_factory
name: 'Pipeline: workflow-factory'
tags:
- layer.pipeline
- topic.pipeline
- pipeline.workflow-factory
- domain.workflow
maturity: draft
summary: 造工作流的工作流 — 输入自然语言需求 → 输出通过全部验证的 LAP 工作流代码
scope: omnicompany
code_anchors:
- src/omnicompany/core/pipelines.py:L532-L557
---

# Pipeline: workflow-factory

> **已知概要**: 造工作流的工作流 — 输入自然语言需求 → 输出通过全部验证的 LAP 工作流代码

## Identity

| Field | Value |
|---|---|
| Pipeline name | `workflow-factory` |
| Domain tag | `workflow` |
